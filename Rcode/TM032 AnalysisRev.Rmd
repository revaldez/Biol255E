...
#"C:/Users/valdezr2/Desktop/RFiles/Data" #set WD
spec <- file.choose()

#spec = ("../Data/Plate1.txt") #set to find .txt file on computer
...
library(rJava)
library(xlsxjars)
write.xlsx(PeakValues, "../Data/PeakValues.xlsx") #Export max values of plate


```
###Strain Analysis
Each bacterial strain tested will have the average, standard deviation and CV calculated. 
###Peak Value
```{r Mean}
###AVC Means
...
print(Mean)
write.xlsx(Mean, "../Data/Mean.xlsx") #Export Mean values of plate
```
...
print(SD)
write.xlsx(SD, "../Data/SD.xlsx") #Export StDev values of plate
...
CV = round(CV,2)
print(CV)
write.xlsx(CV, "../Data/CV.xlsx") #Export CV values of plate
```
...
write.xlsx(TMResults, "../Data/TMResults.xlsx") #Export TMResults values of plate
```

###Box Plot
Plot showing distribution for each strain
```{r Plot }
M1 = Mean[1:4,1:4]

boxplot(M1,  main = "Strain Distribution",
               ylab = "Max RLU",
        col = c("Red", "Purple", "Green", "White"))

```

### % Recovery graph
Graph showing distribution of % recovery for each strain and sample will calculation output for each sample.
...
barplot(Recovery, main = "% Recovery", xlab = "",
        legend = rownames(Recovery),
